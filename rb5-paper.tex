...
The problem considered in this work to showcase RB5 for large-scale exploration is that of exploring a bounded volume, $\mathcal{Q}\subseteq\mathbb{R}^3$ with respect to an inertial navigation frame $\mathcal{O}_W$, deriving collision-free trajectories that avoid $i\in[n],\,n\in\mathbb{N}_{\geq 0}$ obstacles delimited by $\mathcal{Q}^{o_i}\subset\mathbb{R}^3$. We can approximate the space that delimits $\mathcal{Q}$ and $\mathcal{Q}^{o_i}$ for each $i$ with a set of vertices within which the two sets are contained.

\begin{pb}[Exploration]
  Consider sets of vertices $v:=\{v_1,$ $v_2,\dots\}$ and $o_i:=\{o_{i,1},o_{i,2},\dots\}$ with $i\in[n],\,v_j,o_{i,k}\in\mathbb{R}^2,$ $\forall j\in[|v|],\,k\in[|o_i|]$ a point w.r.t. $\mathcal{O}_W$. Let $v$ enclose $\mathcal{Q}$, $o_i$ $\mathcal{Q}^{o_i}$ per each $i$. The \textit{exploration problem} is to find a trajectory that visits each point $\mathbf{p}\in\mathcal{Q}\cap\mathcal{Q}^{o_1}\cap\mathcal{Q}^{o_2}\cap\cdots\cap\mathcal{Q}^{o_n}$.
\end{pb}
