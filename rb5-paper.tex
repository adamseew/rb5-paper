...
%\vspace*{-1cm}
\begin{abstract} 
This systems paper presents the design and implementation of RB5, a wheeled robot for %describes an approach for 
...
show the possible applications and the limitations of the approach. %show improved %autonomy per cost
...
Recent efforts in this direction include low-cost robots for autonomous exploration, %\cite{muller2021openbot, zhou2021smartphone,faisal2021low} 
...
-- and a customized communication protocol 
for human intervention when %in %, e.g.,
%the eventuality of 
the robot %being 
...
%
Although some %traditional and 
less computationally demanding approaches, such as those based on frontiers~\cite{kim2022autonomous,roucek2020darpa,batinovic2021multi}, graphs~\cite{kulkarni2022autonomous,tranzatto2022cerberus,dang2019graph}, grids~\cite{corah2019communication,tabib2022autonomous}, and random trees %\cite{papachristos2017uncertainty} 
are studied, mixed approaches (i.e., a combination of the previous approaches) are %to be 
...
Our methodology is based on a mixed approach. A fron- tier- and sampling-based method that exploits the scarcity of resources while still running %in real-time 
...
%
Being able to operate in both unknown and GPS-denied environments, our %approach
approach derives the 
robot's 
position using %a state-of-the-art 
simultaneous localization and mapping (SLAM)~\cite{labbe2019rtab} % algorithm~\cite{labbe2019rtab} %,campos2021orb}, 
...
The approach is generic in terms of portability to other mobile robots with price and computational constraints but we target use cases where cheaper robotic explorers are preferred, such as nature conservation and surveying efforts~\cite{kirchgeorg2022multimodal} and education~\cite{betz2022autonomous,amster2020turtlebot}. %, where robotic solutions often add to the overall expense. 
...
\begin{figure*}
  ...
  \begin{subfigure}[m]{0.33\textwidth}
    ...
    \label{fig:3-3}
  \end{subfigure}
  \caption{\textbf{Detail of our autonomous exploration methodology}. The %autonomous long-term %large-scale 
...
especially %useful 
in the presence of diverse sensing modalities, e.g., involving raw sensory data, topologies, semantics, etc., and outdoor %, they have multiple %potential 
...
Nevertheless, there are configurations at which there are no feasible trajectories nonetheless, i.e., if $\{\phi_{1,t},\phi_{2,t},\dots\}=\{\varnothing\}$ in Line~\ref{alg:mpty}. In this scenario, %the %framework 
%platform 
our approach allows a human to intervene via %standard 
wireless or LoRa communication. The robot can be then teleoperated over long distances -- studies from the internet-of-things domain~\cite{shanmuga2020survey%,raza2017lora
...
(see Sec.~\ref{sec:fe}). Furthermore, an earlier iteration of the work exploited a different SLAM algorithm from the visual SLAM community~\cite{campos2021orb}, showing that some of the %framework 
%approach 
...
It is composed of three distinct components. A ``ground robot'' ROS2 %\cite{quigley2009ros} 
...
from RB5's onboard computing hardware, an NVIDIA (R) Jetson Xavier NX (TM) embedded board, which implements our autonomous %large-scale 
exploration. %framework
The computing hardware mounts peripherals for sensing the environment and for communication. The former group consists of %a low-cost upward-facing RGB camera for detections (see Sec.~\ref{sec:cf}) and 
%(see Sec.~\ref{sec:le})
