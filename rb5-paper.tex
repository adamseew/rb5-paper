...
The framework evaluates local frontiers at each step, samples the environment, and determines feasible candidate path functions $\phi$ that intersect $\mathcal{Q}^V$ (Definition~\ref{def:pf}).
The next $\phi$ is selected so that the frontier is largest, but  other costs are also possible (see Sec.~\ref{sec:cf}). 
...
\begin{equation}\small
  ...
  \end{bmatrix},
\end{equation}
where $\partial\phi/\mathbf{p}$ is the differntial and $\mathbf{p}_x$, $\mathbf{p}_y$ are the $x$ and $y$ compoenents of $\mathbf{p}$.
It points in the direction where $\phi$ maxi-mally increases. To assign such direction to each point, we use the construct of vector fields similarly to other motion planning literature~\cite{lavalle2006planning,garcia2017guidance,goncalves2010vector}
...
\begin{equation}\small
  ...
  \end{bmatrix},
\end{equation}
is the counterclockwise, $-E$ the clockwise direction~\cite{seewaldphdthesis}.

Let thus the path following of vector field from Eq.~(\ref{eq:vecf}) be 
\begin{equation}
...
\begin{algorithm}[t]
  \begin{algorithmic}[1]
    ...
    \FORALL{$t\in\mathcal{T}$}
      \STATE \textbf{if} $\mathcal{P}\cap\mathcal{Q}=\{\varnothing\}$ \textbf{then return }$\langle\phi,t\rangle$\vspace*{.3ex}
      \STATE $\mathcal{Q}^V_t:=O_{1,t},O_{2,t},\dots,O_{n,t},V_t\gets$ sensors readings\vspace*{.3ex}
      \IF{$\mathcal{Q}_t^V\neq\mathcal{Q}_{t-1}^V$}\vspace*{.3ex}
        \STATE $\{\phi_{1,t},\phi_{2,t},\dots\}\gets$ $\phi$s in Def.~\ref{def:pf}, inters. $\mathcal{Q}^V\cap h(\mathcal{Q}^V_t)$\label{alg:vp}\vspace*{.3ex}
        \STATE \textbf{if} $\phi_t:=\{\phi_{1,t},\phi_{2,t},\dots\}=\{\varnothing\}$ \textbf{then }RB5 is stuck\vspace*{.3ex}
        \STATE \textbf{else}
        ...
        \STATE $\,\,\,\,\,\,\mathcal{P}\gets\mathcal{P}\cup h(\mathcal{Q}_t^V)$
        \STATE \textbf{end if}
        \vspace*{.3ex}
      \ENDIF
      \STATE $\varphi(t,\mathbf{p}(t))\gets\varphi(t-1)+\theta\Delta\phi(\mathbf{p}(t))$ in Eq.~(\ref{eq:pfvf})\vspace*{.3ex}\label{alg:vf}
    \ENDFOR
  \end{algorithmic}
  \caption{Real-time autonomous large-scale exploration}\label{alg}
\end{algorithm}

The path-following vector field is included in the pseudo-code in Algorithm~\ref{alg}, Line~\ref{alg:vf}.

The cost $l$ in Line~\ref{alg:am} is evaluated by taking the function $\phi$ that selects the largest frontier, e.g., if there are no obstavcles $\phi$ is a line that interset RB5 current point $\mathbf{p}$ and the mid-point between the field-of-view. Formally
\begin{equation}\label{eq:cost}\small
  l:=\{\lVert \mathbf{p}_1\hspace*{-.5ex}-\mathbf{p}_2\rVert\,|\,\exists\,\mathbf{p}_1,\mathbf{p}_2\hspace*{-.5ex}\in\hspace*{-.5ex}\mathcal{Q}_t^V\text{ s.t. }\mathbf{p}_1\hspace*{-.5ex}\neq\hspace*{-.5ex}\mathbf{p}_2,\phi(\mathbf{p}_1-\mathbf{p}_2)\approx 0\},
\end{equation}
...
In Line~\ref{alg:vp} the function $h:\mathbb{R}^{2n}\times\mathbb{R}^2\rightarrow\mathbb{R}^{3n}\times\mathbb{R}^3$ maps the vertices to the volume.
