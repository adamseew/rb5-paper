...
\title{A Low-Cost Energy-Efficient Approach for Long-Term %Large-Scale 
...
%Manuscript~submitted~for~publication. Version~\textnumero~1, April~2023
Manuscript in preparation. %\underline{Not for distribution}. 
May~2023
}%
{Seewald \MakeLowercase{\textit{et al.}}: A Low-Cost Approach for Long-Term Exploration}
...
%\vspace*{-1cm}
\begin{abstract} 
This paper tackles %describes an approach for 
autonomous %large-scale and 
long-term exploration for robots with lower sensory footprint. Requiring just an RGB-D camera and low-power computing hardware, %we present an approach based on the open-source robot operating system middleware and 
the approach uses an experimental wheeled robot with rocker-bogie suspension. It operates %on large-scale, 
in unknown and GPS-denied environments, and on indoor and outdoor challenging terrains. 
The exploration path uses %is derived with 
a novel 
...
The approach allows the robot to explore its surroundings at lower update frequencies, utilizing cheaper hardware compared to %state-of-the-art
other approaches, and is generic in terms of portability to other mobile robots with both computational and cost constraints. 
...
%Data
Results show improved %autonomy per cost
...
\IEEEPARstart{T}{he} promise of long-term autonomous robotic exploration is currently being held up in part by the expense of the required sensing, computing, and mechanical hardware. %~\cite{muller2021openbot}. 
This cost is related to the computational intensity of most common navigation and communication approaches~\cite{lluvia2021active,placed2022survey}, which significantly increases for challenging outdoor terrains. Addressing this challenge, we introduce novel techniques to reduce update frequencies and enhance communication capabilities of existing approches. By loosening the required update frequencies and communication requirements, our methods enable the use of lower-performing and lower-cost hardware while still retaining good autonomous performance.
...
compared to state-of-the-art--an RGB depth (RGB-D) camera to sense the environment--and opearates in a variety of terrains. %~(see Figure~\ref{fig}). %both indoors and outdoors.
%
...
Starting from the cost advantages of LoRa communication, we develop a low-cost %approach consisting of an 
...
approach derives robot's 
position using a state-of-the-art simultaneous localization and mapping (SLAM) algorithm~\cite{labbe2019rtab} %,campos2021orb}, 
and the exploration trajectory with a novel methodology that extends exploration literature with a path-following vector field %\cite{goncalves2010vector} 
...
utilizing cheaper computing hardware compared to the state-of-the-art (see Fig.~\ref{fig}).%\cite{placed2022survey,tabib2022autonomous,ebadi2020lamp,dang2019graph}.
...
\end{enumerate*}
%an \textit{approach for autonomous exploration} which extends existing literature to mobile robots \textit{with fewer and cheaper sensors and} works \textit{for long-term exploiting energy-efficient design and low-power communication}.
%In particular, in %In 
%Section~\ref{sec:fe}, data show %improved ``coverage per cost'' 
...
Sec.~\ref{sec:m} describes the approach from the software and hardware standpoints. %Sec.~\ref{sec:rw} summarizes and compares existing literature, 
Sec.~\ref{sec:pf} formalizes the problem of autonomous exploration, and Sec.~\ref{sec:cf} drafts conclusions %a proposes 
...
%exploration 
is that of exploring an unknown bounded space, i.e., of visiting each point in within.
The robot is free to move in the space except for an arbitrary number of obstacles.
...
While the majority exploits the concept of frontiers, %\cite{qiao2019sampling}, 
...
platform allows a human to intervene via standard wireless and LoRa communication technology. The robot can then be teleoperated on long distances--studies from the internet-of-things domain~\cite{shanmuga2020survey%,raza2017lora
...
platform
is distributed under the popular open-source CC BY-NC-SA license\footnote{{\tt\footnotesize \href{https://github.com/adamseew/ytcg_ground-based}{github.com/adamseew/ytcg\_ground-based}}}. It is composed of three distinct components. A ``ground robot'' ROS2 %\cite{quigley2009ros} 
package implements the communication with a base station using either the IEEE 802.11 wireless communication or long-range LoRa technology. The package further implements serial communication with the microcontroller implemented in Arduino and the vertices detection (see Algorithm~\ref{alg}). A ``ground navigation'' ROS package collects point clouds from an RGB-D camera (an Intel (R) RealSense (TM) Depth Camera %~\cite{keselman2017intel} 
...
for low-cost long-term autonomous exploration in both indoor and outdoor challenging environments. While comparable with other literature tackling autonomous exploration, %~\cite{lluvia2021active,placed2022survey}, %,julia2012comparison}, 
...
The exploration is based on a novel mixed approach--a frontier- and sampling-based method from the literature extended with a path-following vector field %~\cite{seewald2022energy,garcia2017guidance,seewaldphdthesis} 
