...
\vspace*{-1cm}
\begin{abstract} 
This paper describes an approach for autonomous %large-scale and 
...
algorithm to derive the position. 
The approach allows the robot to explore its surroundings at lower update frequencies, utilizing cheaper hardware compared to state-of-the-art approaches, and is generic in terms of portability to other mobile robots with both computational and cost constraints. 
The approach further consists of a novel methodology to interact with a remotely located human operator based on an inexpensive long-range and low-power communication technology from the internet-of-things domain (i.e., LoRa) 
...
over the baseline of existing autonomous exploration system-of-systems with a set of indoor and outdoor ``in the field'' experiments (Sec.~\ref{sec:fe}). 
...
\label{sec:cf}
\noindent
The content of this paper consists of an %framework 
...
\section*{Acknowledgments}
\noindent
We thank A.~Michael~West, Victoria~Ereskina, and Beau~Birdsall for their contribution to the mechanical design of the experimental robotic platform used in this paper.
